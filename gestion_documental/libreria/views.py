...
def login(request):
    ...
    cargos_registrados = Cargo.objects.filter(Cargo__isnull=False).distinct()
    ...
    return render(request, 'paginas/login.html', {'formulario':formulario, 'cargos':cargos_registrados})
...
def comision(request):
    pos = 1
    datos = request.session.get('datos')
    libros = dcomision.objects.filter(tipo_cargo = datos) #filter == get
...
def justificaciones(request):
    ...
    datos = request.session.get('datos')
    libros = djustificaciones.objects.filter(tipo_cargo = datos) #filter == get
...
def informes(request):
    ...
    datos = request.session.get('datos')
    libros = dinformes.objects.filter(tipo_cargo = datos) #filter == get
...
def clasificar(request):
    ...
    libros1 = dcomision.objects.filter(registrado = False, tipo_cargo = datos)   #filter (cumple) =! get (si o si) 
    libros2 = djustificaciones.objects.filter(registrado = False, tipo_cargo = datos) 
    libros3 = dinformes.objects.filter(registrado = False, tipo_cargo = datos) 

    libros4 = dvacacionesPEND.objects.filter(registrado = False, tipo_cargo = datos) 
    libros5 = dvacacionesPROX.objects.filter(registrado = False, tipo_cargo = datos) 

    libros6 = dlicenciasREM.objects.filter(registrado = False, tipo_cargo = datos) 
    libros7 = dlicenciasSINREM.objects.filter(registrado = False, tipo_cargo = datos) 
...
def vacaciones(request):
    ...
    if request.method == 'POST':
        ...
        if opcion_seleccionada == 'opcion1':
            pos = 4
            datos = request.session.get('datos')
            libros = dvacacionesPEND.objects.filter(tipo_cargo = datos) #filter == get
            ...
            pos = 5
            datos = request.session.get('datos')
            libros = dvacacionesPROX.objects.filter(tipo_cargo = datos) #filter == get
...
def licencias(request):
    ...
    if request.method == 'POST':
        ...
        if opcion_seleccionada == 'opcion1':
            pos = 6
            datos = request.session.get('datos')
            libros = dlicenciasREM.objects.filter(tipo_cargo = datos) #filter == get
            ...
            pos = 7
            datos = request.session.get('datos')
            libros = dlicenciasSINREM.objects.filter(tipo_cargo = datos) #filter == get
